...
####################################################################################################################
#每个场景道具记录包含以下字段：
#1) 场景道具 ID：用于在其他文件中引用场景道具。每个场景道具 ID 前会自动添加前缀 spr_。
#2) 场景道具标志。可用标志列表请参阅 header_scene_props.py 文件。
#3) 网格名称：网格的名称。
#4) 物理对象名称：
#5) 触发器：与场景道具关联的简单触发器。
####################################################################################################################

on_scene_use_28_days = (ti_on_scene_prop_use,
    [
      (store_trigger_param_1, ":agent_id"),
      (store_trigger_param_2, ":instance_no"),
      
      (prop_instance_get_variation_id, ":var_1", ":instance_no"),
      (neq, ":var_1", 0),
      (prop_instance_get_variation_id_2, ":var_2", ":instance_no"),
      
      (agent_get_player_id, ":player_no", ":agent_id"),
      (try_begin),
        (eq, ":var_1", 1), # spawn item  生成项目
        (assign, ":spawn_item", 0),
        (assign, ":cost", 0),
        (assign, ":random_modifier", 0),
        (assign, ":itm2", 0),
        (assign, ":random_modifier_2", 0),
        (try_begin),
          (eq, ":var_2", 0), # random melee item  随机近战
            (call_script, "script_list_random", "trp_list_random_items"),
            (assign, ":spawn_item", reg1),
            (call_script, "script_list_random", "trp_list_random_item_modifier"),
            (assign, ":random_modifier", reg1),
						(assign, ":cost", "$invasion_random_melee_weapon_cost"),
        (else_try),
          (eq, ":var_2", 1), # random shield item  随机护盾
            (call_script, "script_list_random", "trp_list_random_shields"),
            (assign, ":spawn_item", reg1),
            (call_script, "script_list_random", "trp_list_random_shield_modifier"),
            (assign, ":random_modifier", reg1),
						(assign, ":cost", "$invasion_random_shield_cost"),
        (else_try),
          (eq, ":var_2", 2), # random throw item  随机投掷物品
            (call_script, "script_list_random", "trp_list_random_throw"),
            (assign, ":spawn_item", reg1),
            (call_script, "script_list_random", "trp_list_random_item_modifier"),
            (assign, ":random_modifier", reg1),
            (item_get_type, ":itm_type", ":spawn_item"),
            (try_begin),
              (eq, ":itm_type", itp_type_bow),
                (call_script, "script_list_random", "trp_list_random_arrows"),
                (assign, ":itm2", reg1),
                (call_script, "script_list_random", "trp_list_random_ammo_modifier"),
                (assign, ":random_modifier_2", reg1),
            (else_try),
              (eq, ":itm_type", itp_type_crossbow),
                (call_script, "script_list_random", "trp_list_random_bolts"),
                (assign, ":itm2", reg1),
                (call_script, "script_list_random", "trp_list_random_ammo_modifier"),
                (assign, ":random_modifier_2", reg1),
            (try_end),
						(assign, ":cost", "$invasion_random_throw_weapon_cost"),
        (else_try),
          (eq, ":var_2", 3), # 路障锤
            (assign, ":spawn_item", "itm_hammer"),
						(assign, ":cost", "$invasion_barricade_hammer_cost"), #所需成本
        (else_try),
          (eq, ":var_2", 4), # 食物
            (store_random_in_range, ":spawn_item", "itm_smoked_fish", "itm_siege_supply"),
						(assign, ":cost", "$invasion_food_cost"),
        (try_end),
          ...
          (prop_instance_get_starting_position, pos2, ":instance_no"),
					(position_move_z, pos2, 40),
					(position_move_y, pos2, 50),
					(set_spawn_position, pos2),
					(spawn_item, ":spawn_item", ":random_modifier", 60),
          ...
          (prop_instance_set_position, reg0, pos2),
					...
					(try_begin),
						(gt, ":itm2", 0),
            (position_move_x, pos2, -40),
            (set_spawn_position, pos2),
						(spawn_item, ":itm2", ":random_modifier_2", 60),
					(try_end),
					
					(particle_system_burst, "psys_war_smoke_tall", pos2, 1),
					(val_sub, ":player_gold", ":cost"),
					(player_set_gold, ":player_no", ":player_gold", multi_max_gold_that_can_be_stored),
				(else_try),
					(lt, ":player_gold", ":cost"), 
						(val_sub, ":cost", ":player_gold"),
						(assign, reg1, ":cost"),
						(try_begin),
							(eq, ":var_2", 1),
                (str_store_string, s0, "@a random shield"),
						(else_try),
							(eq, ":var_2", 2),
                (str_store_string, s0, "@a random ranged weapon"),
						(else_try),
							(eq, ":var_2", 3),
                (str_store_string, s0, "@a barricade hammer"),
						(else_try),
                (str_store_string, s0, "@a random melee weapon"),
						(try_end),
            (multiplayer_send_string_to_player, ":player_no", multiplayer_event_show_server_message, "@You need {reg1} more gold to buy {s0}!" ),
				(try_end),
      (else_try),
        (eq, ":var_1", 2),#补充弹药
        (assign, ":cost", "$invasion_ammo_refill_cost"),
			  (try_begin),
          (player_is_active, ":player_no"),
          (player_get_gold, ":player_gold", ":player_no"),
          (ge, ":player_gold", ":cost"), 
            ...
            (val_sub, ":player_gold", ":cost"),
            (player_set_gold, ":player_no", ":player_gold", multi_max_gold_that_can_be_stored),
            (multiplayer_send_string_to_player, ":player_no", multiplayer_event_show_server_message,"@弹 药 已 补 充 ! " ),
			  (else_try),
          (lt, ":player_gold", ":cost"),
            (val_sub, ":cost", ":player_gold"),
            (assign, reg1, ":cost"),
            (multiplayer_send_string_to_player, ":player_no", multiplayer_event_show_server_message,"@You need {reg1} more gold to refill your ammo!" ),
			  (try_end),
      (else_try),
        (eq, ":var_1", 3),#升级装甲
          (try_begin),
            (call_script, "script_player_upgrade_equipment", ":player_no", ":var_2"),
          (try_end),
      (try_end),
      
      (scene_prop_enable_after_time, ":instance_no", 100),
    ])
    # 28 days - 结束
...
##    ]),     
 ...
 ("headquarters_flag_rebel",sokf_moveable|sokf_face_player,"flag_rebel","0", []),
  ...
  ("arabian_ramp_b",sokf_moveable|sokf_destructible,"arabian_ramp_b","bo_arabian_ramp_b", [
    (ti_on_init_scene_prop,
		[
      (store_trigger_param_1, ":instance_no"),
      (scene_prop_set_hit_points, ":instance_no", 999999),
            # (scene_prop_set_slot, ":instance_no", slot_scene_prop_hit_points, 300),
		]),
    ...
    [
      ...
      (try_begin),
      (neg|scene_prop_slot_eq, ":instance_no", slot_scene_prop_is_destroyed, 1),
        (scene_prop_get_slot, ":hit_points", ":instance_no", slot_scene_prop_hit_points),
        (val_sub, ":hit_points", ":damage"),
        (try_begin),
          (le, ":hit_points", 0),
            # (scene_prop_set_slot, ":instance_no", slot_scene_prop_is_destroyed, 1),
            # (prop_instance_play_sound, ":instance_no", "snd_dummy_destroyed"),
            (agent_play_sound, ":agent_id", "snd_dummy_destroyed"),
            (call_script, "script_destroy_barricade", ":instance_no"),
            # todo
        (else_try),
          # (play_sound_at_position, "snd_dummy_hit", pos1),
          # (prop_instance_play_sound, ":instance_no", "snd_dummy_hit", sf_always_send_via_network),
          (agent_play_sound, ":agent_id", "snd_dummy_hit"),
          (particle_system_burst, "psys_dummy_smoke", pos1, 3),
          (particle_system_burst, "psys_dummy_straw", pos1, 10),    
